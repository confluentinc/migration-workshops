...
# Cutover script for switching the producer/consumer between environments
# (oss -> gateway -> cc). During the workshop you normally run through the Gateway
# the whole time; this helper is handy for testing each environment directly.
...
# Prefer the workshop virtualenv's Python if present (created by setup.sh), so this
# helper works whether or not the venv is currently activated.
PY="python3"
...
start_consumer() {
    ...
    source "$env_file"
    nohup "$PY" orders_consumer.py > "$LOG_DIR/consumer.log" 2>&1 &
...
start_producer() {
    ...
    source "$env_file"
    nohup "$PY" orders_producer.py > "$LOG_DIR/producer.log" 2>&1 &
...
cutover() {
    ...
    source "$env_file"
    "$PY" create_topic.py orders || warning "Topic creation failed or topic already exists"
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  cutover <env>  - Switch to specified environment (oss, gateway, cc)"
    echo "  status         - Show current process status"
    echo "  stop           - Stop all processes"
    echo ""
    echo "Examples:"
    echo "  $0 cutover gateway   # Run clients through the Gateway"
    echo "  $0 cutover cc        # Connect clients directly to Confluent Cloud"
    echo "  $0 cutover oss       # Connect clients directly to local OSS Kafka"
...
case "$1" in
    cutover)
        ...
        if [[ "$2" != "oss" && "$2" != "gateway" && "$2" != "cc" ]]; then
            error "Invalid environment: $2"
            error "Valid environments: oss, gateway, cc"
            exit 1
        fi
